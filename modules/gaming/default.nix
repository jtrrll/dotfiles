          ...
          (lib.mkIf config.programs.retroarch.enable {
            home.file = {
              ...
              globalShader = {
                ...
                source = self.packages.${pkgs.stdenv.hostPlatform.system}.crt-shader;
              };
              gambatteShader = {
                target = ".config/retroarch/config/Gambatte/Gambatte.slangp";
                source = self.packages.${pkgs.stdenv.hostPlatform.system}.gbc-shader;
              };
              melondsShader = {
                target = ".config/retroarch/config/melonDS/melonDS.slangp";
                source = self.packages.${pkgs.stdenv.hostPlatform.system}.ds-shader;
              };
              mgbaShader = {
                target = ".config/retroarch/config/mGBA/mGBA.slangp";
                source = self.packages.${pkgs.stdenv.hostPlatform.system}.gba-shader;
              };
              ppssppShader = {
                target = ".config/retroarch/config/PPSSPP/PPSSPP.slangp";
                source = self.packages.${pkgs.stdenv.hostPlatform.system}.psp-shader;
              };
            };
            programs = {
              retroarch = {
