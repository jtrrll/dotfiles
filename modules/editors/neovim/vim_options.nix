...
}:
{
  config = {
    programs.nixvim.opts = {
      ...
      colorcolumn = lib.concatStringsSep "," (map toString config.editors.lineLengthRulers); # highlight columns
      ...
      scrolloff = config.editors.linesAroundCursor; # number of lines to preserve above/below the cursor
      shiftround = true; # indent to nearest indent level
      shiftwidth = config.editors.indentWidth; # spaces per indent level
      ...
      tabstop = config.editors.indentWidth; # number of spaces per tab
